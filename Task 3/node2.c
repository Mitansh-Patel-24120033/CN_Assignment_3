...
void rtinit2() {
    // Initialize direct costs: [3, 1, 0, 2]
    for(int i = 0; i < 4; i++) {
        for(int j = 0; j < 4; j++) {
            dt2.costs[i][j] = 999;
        }
    ...
    dt2.costs[2][2] = 0;
    // Set direct links
    dt2.costs[0][0] = 3;  // Node 2->0
    dt2.costs[1][1] = 1;  // Node 2->1
    dt2.costs[3][3] = 2;  // Node 2->3
...
void rtupdate2(struct rtpkt *rcvdpkt) {
    ...
    for(int dest = 0; dest < 4; dest++) {
        ...
        int new_cost = rcvdpkt->mincost[dest] + dt2.costs[source][source];
    ...
    printf("\nNode2 updated via Node%d:", source);
    }
    printdt2(&dt2); 
}
...
void send_to_neighbors2() {
    ...
    for(int dest = 0; dest < 4; dest++) {
        ...
        int via = -1;
        for(int v = 0; v < 4; v++) {
            if(dt2.costs[dest][v] < min) {
                min = dt2.costs[dest][v];
                via = v;
            }
        }
        mincost[dest] = min;
